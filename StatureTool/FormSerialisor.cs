      ...
      foreach (Control childCtrl in c.Controls) {
        if (!(childCtrl is Label)) {
          ...
          if (childCtrl is TextBox) {
            ...
            xmlSerialisedForm.WriteElementString("Value", ((DateTimePicker)childCtrl).Value.ToString());
          }
          ...
          if (childCtrl.HasChildren) {
            ...
            {
              //don't go into subelements of NumericUpDown
            }
            else {
          ...
          if (ctrlToSet.GetType().ToString() == controlType) {
            ...
            switch (controlType) {
              ...
              case "System.Windows.Forms.ListBox":
                ...
                XmlNodeList xnlItems = n.SelectNodes("Items");
                lst.Items.Clear();      
                for (int i = 0; i < xnlItems.Count; i++) {
                  lst.Items.Add(xnlItems[i].InnerText);
                }
              ...
              case "System.Windows.Forms.DateTimePicker":
                ((System.Windows.Forms.DateTimePicker)ctrlToSet).Value = Convert.ToDateTime(n["Value"].InnerText);
                break;
              case "System.Windows.Forms.NumericUpDown":
                ((System.Windows.Forms.NumericUpDown)ctrlToSet).Value = Convert.ToInt16(n["Value"].InnerText);
                break;
             }
